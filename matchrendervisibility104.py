...
# This is made to add in the ability to show render only from the 
# 3D View and the ability to match visibility toggles with 
# all collections and objects accordingly and recursibley.

# Update: changed the way the UI would show in the outliner, now it is in the Filter and compatible with Blender and Bforartists. 
...
def register():
    ...
    if bpy.types.OUTLINER_MT_view.is_registered == False:
